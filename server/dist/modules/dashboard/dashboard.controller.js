"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.getVendorStats = exports.getAdminStats = void 0;
const mongoose_1 = __importDefault(require("mongoose"));
const Delivery_1 = __importDefault(require("../../models/Delivery"));
const Product_1 = __importDefault(require("../../models/Product"));
const Vendor_1 = __importDefault(require("../../models/Vendor"));
const Role_1 = __importDefault(require("../../models/Role"));
const UserRole_1 = __importDefault(require("../../models/UserRole"));
// @desc    Get admin overview stats
// @route   GET /api/dashboard/admin
// @access  Private (Admin)
const getAdminStats = async (req, res, next) => {
    try {
        const customerRole = await Role_1.default.findOne({ name: 'customer' });
        let totalUsers = 0;
        if (customerRole) {
            totalUsers = await UserRole_1.default.countDocuments({ roleId: customerRole._id });
        }
        const totalOrders = await Delivery_1.default.countDocuments();
        const pendingVendors = await Vendor_1.default.find({ approvalStatus: 'pending' }).limit(5).populate('userId', 'name phone email');
        // Mocking revenue for now since we don't have a generic "Order" or "Payment" model fully tracking all inflows yet
        const totalRevenue = 124500;
        res.status(200).json({
            data: {
                totalRevenue,
                totalUsers,
                totalOrders,
                pendingVendors
            }
        });
    }
    catch (error) {
        next(error);
    }
};
exports.getAdminStats = getAdminStats;
// @desc    Get vendor dashboard stats
// @route   GET /api/dashboard/vendor/:vendorId
// @access  Private (Vendor)
const getVendorStats = async (req, res, next) => {
    try {
        const { vendorId } = req.params;
        const vIdString = Array.isArray(vendorId) ? vendorId[0] : vendorId;
        let targetVendorId = vIdString;
        if (!vIdString || !mongoose_1.default.Types.ObjectId.isValid(vIdString)) {
            const firstVendor = await Vendor_1.default.findOne();
            if (firstVendor) {
                targetVendorId = firstVendor._id.toString();
            }
        }
        const lowStockProducts = await Product_1.default.find({ vendorId: targetVendorId, stockQuantity: { $lt: 10 } });
        const activeProducts = await Product_1.default.countDocuments({ vendorId: targetVendorId, isActive: true });
        // Mocking recent orders
        const recentOrders = [
            { id: 'ORD-2021', items: 3, total: 450, status: 'Packing' },
            { id: 'ORD-2022', items: 1, total: 60, status: 'Packing' },
            { id: 'ORD-2023', items: 2, total: 180, status: 'Packing' },
        ];
        res.status(200).json({
            data: {
                todaySales: 12450,
                activeSubscriptions: 450,
                pendingOrders: 45,
                lowStockProducts,
                recentOrders
            }
        });
    }
    catch (error) {
        next(error);
    }
};
exports.getVendorStats = getVendorStats;
